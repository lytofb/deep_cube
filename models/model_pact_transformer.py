...
class RubikGPT(nn.Module):
    ...
    def __init__(self,
        ...
        # 用 nn.Embedding 代替 nn.Linear
        # state_emb 用于每个贴纸（状态）嵌入，输入是离散的数值（0～num_colors-1）
        self.state_emb = nn.Embedding(num_colors, d_model)
        # action_emb 用于离散动作
    ...
    def forward(self, src):
        ...
        state  = src[:, :, :54].long()
        action = src[:, :, 54:].long()   # 最后一维 (B,T,1)

        # 2) 分别查 embedding
        # 对于 state，每个时间步有 54 个离散 token，查 embedding后再汇聚（如取平均）成一个表示
        state_tokens = self.state_emb(state)  # (B, T, 54, d_model)
        state_emb = state_tokens.mean(dim=2)  # (B, T, d_model)

        # 对于 action，直接查 embedding
        action_emb = self.action_emb(action)  # (B, T, 1, d_model)
        action_emb = action_emb.squeeze(2)
...
if __name__ == "__main__":
    ...
    # history_len+1 = 5 比如
    T = 5
    dummy_src = torch.randint(0,6,(B, T, 55))  # (B, T, 55)
